        ...
        body: Column(
          children: [
            ...
            Expanded(
              ...
              child: fetchData
                  ? FutureBuilder<Data>(
                key: ValueKey(fetchTrigger), // Ensure the widget rebuilds when the fetchTrigger changes
                future: fetcher(),
                builder: (_, snapshot) {
                  ...
                  return Container(
                    ...
                    child: ScrollableTableView(
                      headers: [
                        ...
                        label: label,
                        width: label == "Sprint" || label == "Chi phí" ? 200 : null,
                      )).toList(),
                      rows: snapshot.data!.items.map((cost) {
                        return [
                          (++index).toString(),
                          cost.expenseObj.id,
                          formatDate(cost.expenseObj.time),
                          cost.expenseObj.category,
                          "${cost.expenseObj.amount} VNĐ"
                        ];
                      }).map((record) {
                        return TableViewRow(
                          height: 60,
                          cells: record.map((value) {
                            return TableViewCell(
                              child: Text(value.toString(), maxLines: 1, overflow: TextOverflow.ellipsis),
                            );
                          }).toList(),
                        );
                      }).toList(),
                  ...
                  : const Center(child: Text('Hãy "Tìm kiếm" để tải dữ liệu')),
            ),
          ],
        ),
      ),
